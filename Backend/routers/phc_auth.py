...
def _init_db():
    try:
        ...
        # do NOT raise here automatically at import time in production/dev;
        # raising prevents the app from starting if DB is temporarily unavailable.
        # raise
    finally:
        try:
            cur.close()
...
async def signup(request: Request):
    ...
    # ensure bcrypt length limit and hash via hashing helper
    password = data.password[:72]
    hashed_pw = Hash.hashing(password)
...
async def login(request: Request):
    ...
    # verify via hashing helper
    if not Hash.verifying(password[:72], stored_pw):
        raise HTTPException(status_code=401, detail="Invalid email or password")
